...
public class CuriosContainerHandler
implements IGuiContainerHandler<CuriosScreen> {
    @Nonnull
    public List<Rect2i> getGuiExtraAreas(CuriosScreen containerScreen) {
        LocalPlayer player = containerScreen.getMinecraft().f_91074_;
        if (player != null) {
            return CuriosApi.getCuriosInventory((LivingEntity)containerScreen.getMinecraft().f_91074_).map(handler -> {
                int width;
                ...
                RecipeBookComponent guiRecipeBook = containerScreen.m_5564_();
                if (guiRecipeBook.m_100385_()) {
                    ...
                    for (RecipeBookTabButton tab : guiRecipeBook.f_100279_) {
                        if (!tab.f_93623_) continue;
