...
implements ISlotHelper {
    ...
    public SortedMap<ISlotType, ICurioStacksHandler> createSlots() {
        TreeMap<ISlotType, ICurioStacksHandler> curios = new TreeMap<ISlotType, ICurioStacksHandler>();
        this.getSlotTypes().forEach(type -> curios.put((ISlotType)type, new CurioStacksHandler(null, type.getIdentifier(), type.getSize(), type.isVisible(), type.hasCosmetic(), type.canToggleRendering(), type.getDropRule())));
        return curios;
    }
    ...
    public SortedMap<ISlotType, ICurioStacksHandler> createSlots(LivingEntity livingEntity) {
        TreeMap<ISlotType, ICurioStacksHandler> curios = new TreeMap<ISlotType, ICurioStacksHandler>();
        CuriosApi.getCuriosHelper().getCuriosHandler(livingEntity).ifPresent(handler -> this.getSlotTypes().forEach(type -> curios.put((ISlotType)type, new CurioStacksHandler((ICuriosItemHandler)handler, type.getIdentifier(), type.getSize(), type.isVisible(), type.hasCosmetic(), type.canToggleRendering(), type.getDropRule()))));
        return curios;
    }
    ...
    public int getSlotsForType(@Nonnull LivingEntity livingEntity, String identifier) {
        return CuriosApi.getCuriosHelper().getCuriosHandler(livingEntity).map(handler -> handler.getStacksHandler(identifier).map(ICurioStacksHandler::getSlots).orElse(0)).orElse(0);
    }
    ...
    public void unlockSlotType(String id, LivingEntity livingEntity) {
        CuriosApi.getCuriosHelper().getCuriosHandler(livingEntity).ifPresent(handler -> {
            if (handler.getStacksHandler(id).map(stacksHandler -> stacksHandler.getSlots() == 0).orElse(false).booleanValue()) {
